...
| P1.7-T04 | CLI UX | Show stats and context policy in CLI output | P1.7-T03 | done | Claude Opus 4.7 | `--show-stats` prints stable timing/memory/context fields; `--context-policy` works | implemented 2026-06-04 by Claude Opus 4.7. Changes: [tiny_duo_infer/cli.py](tiny_duo_infer/cli.py) adds `--context-policy {allow_context_stop,reject,truncate_left,truncate_right,reserve_generation}` (default `allow_context_stop`) forwarded to `GenerationRequest` for both `--prompt` and `--message` paths; `main()` gains a `stderr: TextIO = sys.stderr` parameter; `--show-stats` now writes the full Phase 1.7 spec block (14 fields, one `key=value` per line, in spec order) to stderr — generated text remains the only thing on stdout, satisfying spec line 391-392 ("Generated text must remain on stdout. Stats must be written to stderr"); when `response.stats is None` (legacy fakes) the CLI falls back to the Phase 1.6 single-line shape on stderr. Float fields use `:.2f` for stable test substring matching; ints render unformatted. New helper `_format_stats_lines()` keeps the field list in one place. Tests: [tests/test_cli.py](tests/test_cli.py) extends `_FakeEngine` with `next_stats` injection and a `_make_fake_stats()` helper that satisfies all `GenerationStats` invariants; existing `--show-stats` test split into 3 tests (full block on stderr, one-field-per-line ordering, fallback when stats is None); 4 new context-policy tests cover all 5 policies forwarded, default `allow_context_stop`, `--message` mode forwarding, and unknown policy `SystemExit` before model load; slow Qwen3 smoke test updated to capture stderr; reviewed and signed off by codex on 2026-06-04; `uv run pytest tests/test_cli.py -q`: 32 passed, 2 skipped; `uv run pytest -q`: 366 passed, 11 skipped. Spec compliance: stats fields = exactly the 14 listed in spec line 376-389; flag name + choices match spec line 397; default matches spec line 400; stderr routing matches spec line 391-392. T03 confirmed `GenerationResponse.stats` is populated on every real engine path, so the expanded block fires in production. Coordinated with `cc` on T05: non-overlapping files (T05 = `serving/api.py` + `tests/test_serving.py`). |
