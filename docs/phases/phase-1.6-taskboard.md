...
| P1.6-T06 | Serving | Single-request HTTP API | P1.6-T02, P1.6-T04 | done | cc | `/generate`, `/generate/stream`, and `/health` work with fake-engine tests | reviewed by codex; FastAPI app in `tiny_duo_infer/serving/api.py`; `threading.Lock` serializes requests (non-blocking — busy → 503); engine resolved before lock to prevent lock leak; `POST /generate` runs engine in executor; `POST /generate/stream` yields NDJSON (`{"done":false,"text":"…"}` fragments + `{"done":true,"text":"<full>",…metadata}` final); `_run_generation()` checks stop strings before yielding; duplicate `Iterator` import removed; `httpx2` replaced with `httpx`; starlette deprecation warning suppressed in pytest config; 12 unit tests include lock-leak regression; `uv run pytest tests/test_serving.py -q`: 12 passed, 2 skipped; `uv run pytest -q`: 278 passed, 12 skipped; no findings |
| P1.6-T07 | Close | Real-model verification and handoff | P1.6-T05, P1.6-T06 | done | cc | pytest passes; real Llama/Qwen3 smoke results are recorded or skipped with reasons | reviewed by codex; InferenceWorker (`tiny_duo_infer/serving/worker.py`) added: dedicated background thread owns Engine and all MLX GPU ops; `from_engine(engine)` for tests, `from_path(model_path, max_seq_len)` for production (engine init inside worker thread); routes now async (event loop awaits asyncio.Future / asyncio.Queue, no MLX on event-loop thread); `threading.Lock` removed in favour of worker's internal `busy_event`; CLI and HTTP server have fully separate engine lifecycles; `create_app_from_path()` added for production and slow smoke tests; FastAPI `_lifespan` hook added — no-op on startup, calls `worker.shutdown()` on server stop so inference thread exits cleanly; smoke tests use plain `TestClient` (no `anyio.run()` workaround); `uv run pytest --run-slow -q`: 285 passed, 5 skipped; `uv run pytest tests/test_serving.py --run-slow -v`: 14/14 passed; no findings |
