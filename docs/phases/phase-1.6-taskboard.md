...
| P1.6-T07 | Close | Real-model verification and handoff | P1.6-T05, P1.6-T06 | review | cc | pytest passes; real Llama/Qwen3 smoke results are recorded or skipped with reasons | `/generate/stream` converted to async generator (MLX thread-affinity fix); slow smoke tests updated to use `httpx.AsyncClient + ASGITransport` inside `anyio.run()` so engine init and ASGI handler share the same thread (starlette TestClient spawns a separate portal thread that breaks MLX GPU stream); `uv run pytest --run-slow -q`: 285 passed, 5 skipped; `uv run pytest tests/test_serving.py --run-slow -v`: 14/14 passed; 5 skips are pre-existing infrastructure skips unrelated to Phase 1.6 |
