...
| P1.8-T08 | Docs | README, architecture, file-structure, strategy, and learning docs updates | P1.8-T04, P1.8-T05, P1.8-T06, P1.8-T07 | done | Claude Opus 4.7 | public usage and learning notes describe weight-only quantization | docs-only update bringing public surfaces into agreement with T01–T07. README: corrected `--show-stats` field count 14→21 and HTTP stats 19→26, added `--quantization`/`--quant-group-size` rows to local CLI, HTTP startup, and profiling sections, added INT4 Llama and INT8 HTTP examples, documented profiling JSON `schema_version=2`. `docs/architecture.md`: added Implementation Boundary row delegating to `mx.quantize`/`mx.quantized_matmul`, prohibited `mlx.nn.QuantizedLinear`/`mlx.nn.quantize`, added `quantization.py` and `weights/quantizer.py` to package structure and module-responsibility table, refined Phase 1.6/1.7/1.8 table, replaced blanket "quantization out of scope" with precise weight-only-only boundary. `docs/file-structure.md`: expanded source rows to call out new modules and worker-thread forwarding; tests row references new quantization tests. `docs/refined-plan.md`: tightened Phase 1.8 decision line and added bullet listing public-surface additions. `learning_materials/roadmap.md`: intro now includes Phase 1.8; added new section 22 "Weight-Only Quantization" with reading list, learning objectives, and try-it exercises; renumbered handoff section to 23 and extended its reading/learning bullets; added Phase 1.8 sources to suggested reading order. New `learning_materials/deep_dives/quantization.md`: concept walkthrough (why weight-only, affine grouping math, fused matmul rationale, why `mx.dequantize` restricted, eligibility code, Llama tied lm_head handling, Qwen3 explicit lm_head, `LinearWeightStats` accounting, why throughput is measured not gated, loading pipeline, verification checklist). Field counts cross-checked against `tiny_duo_infer/cli.py:_format_stats_lines` (21) and `tiny_duo_infer/serving/api.py:GenerationStatsBody` (26). reviewed and signed off by codex after round-2 fixes: roadmap now distinguishes Phase 1.7 baseline 14-line stats/schema v1 from Phase 1.8 21-line stats/schema v2, and the quantization deep dive loading pipeline now matches `engine.py:130-157` with `compute_linear_weight_stats(project_weights)` before model construction/loading. `uv run pytest -q`: 509 passed, 13 skipped; `uv run python scripts/profile_generation.py --help`: exits 0 and shows `--quantization {none,int4,int8}` plus `--quant-group-size QUANT_GROUP_SIZE`. Project-wide grep for stale `14 fields`/`14 (lines\|output)`/`schema_version=1` matches only intentional historical references. `learning_materials/internals.md`, `learning_materials/data-flow.md`, the `_zh.md` translations, and `roadmap_cc.md` were intentionally left untouched because they focus on Phase 1 fundamentals unchanged by weight-only quantization. |
